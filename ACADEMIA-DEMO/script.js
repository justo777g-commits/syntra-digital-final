...
(function () {
  ...
  // Fire/Gym color palette: red, orange, amber, grey
  const COLORS = [
    'rgba(229,57,53,',
    'rgba(255,112,67,',
    'rgba(255,167,38,',
...
(function () {
  const sections = document.querySelectorAll(
    '.sobre-content, .sobre-visual, .contato-inner, .section-header, .localizacao-info, .localizacao-mapa'
  );
...
// ── CARD MAGNETIC HOVER ───────────────────────────────────────
document.querySelectorAll('.service-card, .trainer-card, .plano-card, .avaliacao-card').forEach(card => {
...
console.log('%c SYNTRA ACADEMY ', 'background: linear-gradient(135deg, #E53935, #FF7043); color: #fff; font-size: 14px; font-weight: bold; padding: 6px 12px; border-radius: 6px;');
console.log('%c syntraacademy.com.br — Academia Premium · Resultados Reais ', 'color: #FFA726; font-size: 11px;');
